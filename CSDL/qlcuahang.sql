...
------------------Phieu Su co--------------------
IF EXISTS(SELECT name FROM sys.objects WHERE name = N'AUTO_IDPhieuSuCo')
DROP FUNCTION dbo.AUTO_IDPhieuSuCo;
GO
	CREATE FUNCTION dbo.AUTO_IDPhieuSuCo()
	RETURNS VARCHAR(6)
	AS
	BEGIN
		DECLARE @ID VARCHAR(6)
		IF (SELECT COUNT(MAPHIEUSUCO) FROM PHIEUSUCO) = 0
			SET @ID = '001'
		ELSE
		BEGIN
			SELECT @ID = MAX(RIGHT(MAPHIEUSUCO, LEN(MAPHIEUSUCO) - 3)) FROM PHIEUSUCO;
			SET @ID = @ID + 1
			WHILE LEN(@ID) < 3 
			BEGIN 
				SET @ID = '0' + @ID 
